"""
aipartnerupflow API Executor for calling other aipartnerupflow instances

This executor allows tasks to call other aipartnerupflow API services
...
class ApFlowApiExecutor(BaseTask):
    """
    Executor for calling other aipartnerupflow API instances
    ...
    name = "aipartnerupflow API Executor"
    description = "Call other aipartnerupflow API instances for distributed execution"
    tags = ["apflow", "api", "distributed", "orchestration"]
    examples = [
        "Call remote aipartnerupflow instance",
    ...
    async def execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute an aipartnerupflow API call
        
        Args:
            inputs: Dictionary containing:
                - base_url: aipartnerupflow API base URL (e.g., "http://localhost:8000") (required)
        ...
        logger.info(f"Calling aipartnerupflow API {method} on {base_url}")
        
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                # Check for cancellation before making request
                if self.cancellation_checker and self.cancellation_checker():
                    logger.info("aipartnerupflow API call cancelled before execution")
                    return {
                        "success": False,
                ...
                if self.cancellation_checker and self.cancellation_checker():
                    logger.info("aipartnerupflow API call cancelled after request")
                    return {
                        "success": False,
                        "error": "Call was cancelled",
                        "base_url": base_url,
                ...
                if response.status_code != 200:
                    logger.error(
                        f"aipartnerupflow API returned status {response.status_code}: {response.text}"
                    )
                    return {
                        "success": False,
                ...
                if "error" in json_response:
                    ...
                    logger.error(f"aipartnerupflow API error: {error}")
                    return {
                        "success": False,
            ...
            logger.error(f"aipartnerupflow API call timeout after {timeout} seconds: {base_url}")
            return {
                "success": False,
            ...
            logger.error(f"aipartnerupflow API request error: {e}", exc_info=True)
            return {
                "success": False,
            ...
            logger.error(f"Unexpected error calling aipartnerupflow API: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
                "base_url": base_url,
    ...
    async def _wait_for_task_completion(
        ...
        while True:
            ...
            if not poll_success and consecutive_failures > 0:
                ...
                current_backoff = min(
                    base_backoff * (2 ** (consecutive_failures - 1)),
                    max_backoff
                )
                logger.debug(
    ...
    def get_input_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                "base_url": {
                    "type": "string",
                    "description": "aipartnerupflow API base URL (e.g., 'http://localhost:8000')"
                },
                "method": {
                    "type": "string",
                    "description": "API method name (e.g., 'tasks.execute', 'tasks.create', 'tasks.get')"
                },
                "params": {
                    "type": "object",
                ...
                "auth_token": {
                    "type": "string",
                    "description": "JWT token for authentication (if API requires it)"
                },
                "use_streaming": {
                    "type": "boolean",
                    "description": "Use streaming mode (only for tasks.execute, default: False)"
                },
                "wait_for_completion": {
                    "type": "boolean",
                    "description": "Wait for task completion (only for tasks.execute, default: False)"
                },
                "poll_interval": {
                    "type": "number",
