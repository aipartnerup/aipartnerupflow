...
```
aipartnerupflow/
├── src/aipartnerupflow/     # Main source code
│   ├── core/                # Core framework
...
- **Python 3.9+** (3.12+ recommended, see note below)
...
python -m aipartnerupflow.main

# Method 3: Using uvicorn directly
uvicorn aipartnerupflow.main:app --reload --port 8000
```
...
Includes:
- `beautifulsoup4` - Web parsing (for example implementations)
- `scikit-learn` - Machine learning utilities (for example implementations)

**Purpose**: Provides predefined example crews, batches, and custom tasks for users to learn from and customize.

**Note**: Renamed from `[ext]` to `[examples]` for clarity. The `ext/` or `extensions/` naming is reserved for future third-party extension/plugin system.
...
├── test_agent_executor.py   # AgentExecutor tests
├── test_task_model.py       # TaskModel tests
└── INTEGRATION_COMPARISON.md # Integration comparison docs
```
...
**Extension Modules**:
- **`ext/`**: Predefined examples [ext extra]
...
**Service Modules**:
- **`api/`**: API layer (A2A server, handlers) [api extra]
