...
class TestTaskManager:
    ...
    async def test_task_manager_with_hooks(self, sync_db_session):
        ...
        async def pre_hook(task):
            pre_hook_called.append((task.id, task.input_data))
            # Modify task.input_data to demonstrate hook can transform data
            if task.input_data and "url" in task.input_data:
                task.input_data["url"] = task.input_data["url"].strip()
        
        async def post_hook(task, input_data, result):
            post_hook_called.append((task.id, input_data, result))
        ...
        task = await task_manager.task_repository.create_task(
            ...
            schemas={"type": "crew", "method": "web_analyzer"}
        )
        ...
        # Verify input_data was modified by pre-hook
        # Note: The actual input_data modification happens in the hook
    ...
    async def test_task_manager_with_sync_hooks(self, sync_db_session):
        ...
        def sync_post_hook(task, input_data, result):
            post_hook_called.append((task.id, input_data, result))
    ...
    async def test_task_manager_hooks_error_handling(self, sync_db_session):
        ...
        async def failing_post_hook(task, input_data, result):
            post_hook_called.append((task.id, input_data, result))
    ...
    async def test_create_task(self, sync_db_session):
        ...
        assert task.schemas["method"] == "web_analyzer"
